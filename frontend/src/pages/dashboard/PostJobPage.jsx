...
const EMPTY_FORM = {
  ...
  workType: "On-site",
  location: "",
  jobType: "",            // Full-time | Intern | Part-time | Contract
  salaryMax: "",
  skills: "",
  description: "",
  requirements: [],
...
export default function PostJobPage({ token, onCreated }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      // Payload: salaryMax will be formatted with ₱ by the backend,
      // but we can also send the raw number; controller will format safely.
      const payload = {
        title: form.title.trim(),
        workType: form.workType,
        location: form.location.trim(),
        jobType: form.jobType,
        salaryMax: Number(form.salaryMax), // backend will store as "₱<amount>"
        skills: form.skills
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean),
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      >
        ...
        {/* Salary Max only, with peso sign prefix (UI) */}
              ...
              {form.requirements.map((it, idx) => (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => removeAt("requirements", idx)}
                  >
                    remove
                  </button>
                </li>
              ...
              {form.responsibilities.map((it, idx) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    className="text-red-600 hover:underline"
                    onClick={() => removeAt("responsibilities", idx)}
                  >
                    remove
                  </button>
                </li>
        ...
        <div className="md:col-span-2">
          <Field
            label="What the company offers (list)"
            error={errors.offers}
            required
          >
            <div className="flex gap-2">
