...
export default function Notifications() {
  ...
  const [filter, setFilter] = useState("all"); // all | unread | application | status
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "application") data = data.filter((n) => n.type === "application");
  ...
  async function fetchNotifications({ showSpinner = true } = {}) {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/company/notifications?limit=${PAGE_SIZE}&page=${page}`,
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Stay on top of new applicants and status updates
            </p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="p-10 border border-dashed border-gray-300 rounded-xl text-center bg-gray-50">
          ...
          <p className="text-sm text-gray-500">You’ll see new applicants and updates here.</p>
        </div>
      ) : (
        <div className="grid gap-3">
          {filtered.map((n) => {
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Chip intent={tagIntent}>{n.status || "—"}</Chip>
